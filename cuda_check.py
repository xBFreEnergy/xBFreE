...
def get_cuda_devices():
    ...
    if result != CUDA_SUCCESS:
        cuda.cuGetErrorString(result, ctypes.byref(error_str))
        logging.error("cuDeviceGetCount failed with error code %d: %s" % (result, error_str.value.decode()))
        return
    ...
    for i in range(nGpus.value):
        ...
        if result != CUDA_SUCCESS:
            cuda.cuGetErrorString(result, ctypes.byref(error_str))
            logging.error("cuDeviceGet failed with error code %d: %s" % (result, error_str.value.decode()))
            return
        ...
        if cuda.cuDeviceGetAttribute(ctypes.byref(cores), CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT, device) == CUDA_SUCCESS:
            ...
            logging.debug("  CUDA Cores: %s" % (cores.value * ConvertSMVer2Cores(cc_major.value, cc_minor.value) or
                                           "unknown"))
        ...
        if result != CUDA_SUCCESS:
            cuda.cuGetErrorString(result, ctypes.byref(error_str))
            logging.error("cuCtxCreate failed with error code %d: %s" % (result, error_str.value.decode()))
        else:
            try:
            ...
            if result == CUDA_SUCCESS:
                ...
                cuda.cuGetErrorString(result, ctypes.byref(error_str))
                logging.error("cuMemGetInfo failed with error code %d: %s" % (result, error_str.value.decode()))
