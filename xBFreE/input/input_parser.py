...
class Variable(object):
    ...
    def __init__(self, varname, dat_type=int, default=None, description=''):
        ...
        """
        # Catch illegalities
        if dat_type not in (int, str, float, list[str], list[int], list[float]):
            raise InputError('Variable has unknown data type %s' % dat_type.__name__)

        self.name = varname
        self.datatype = dat_type
        # check if this variable is a compose one, i.e. list[float]
        self.int_datatype = None
        if dat_type in (list[str], list[int], list[float]):
            self.int_datatype = dat_type.__args__[0]
        if default is None:
            self.value = None
        elif self.datatype is str:
            self.value = default.replace("'", '').replace('"', '')
        elif self.int_datatype:
            self.value = [self.int_datatype(x) for x in default]
        else:
...
class Namelist(object):
   ...
   """
    ...
    def __init__(self, trigger, name, full_name):
    ...
    def addVariable(self, varname, datatype, default=None, description=None):
        ...
        self.variables[varname] = Variable(varname, datatype, default, description)
...
class InputFile(object):
   ...
   """
    ...
    def print_contents(self, destination, calc_list=None):
        ...
        if calc_list:
            dest.write(f'Input file generated by gmx_MMPBSA ({__version__})\n'
                       ...
                       f'you obtain.\n\n')
        for namelist, description in self.namelists_info.items():
    ...
    def addNamelist(self, name, full_name, description, variable_list, trigger=None):
        ...
        if name in self.namelists_info:
            raise InputError('Namelist %s defined multiple times' % name)

        self.namelists_info[name] = description
        self.namelists[name] = Namelist(trigger, name, full_name)
    ...
    def _namelist_name(self, nml):
      ...
      """
        ...
        for key in self.namelists_info:
    ...
    def Parse(self, filename):
        ...
        for line in lines:
            ...
            # Now if we finally find a namelist
                ...
                namelist = self._namelist_name(namelist)
        ...
        INPUT = {}
        for nml in self.namelists_info:
            INPUT[nml] = {}
